...
select 학번,substr(이름,1,1) '성' /*SELECT SUBSTR(name, 1, 1) ‘성’, COUNT(*) ‘인원’ *//*select 학번, 이름, left(이름,1) '성'*/
from 학생;

/*37 〈학생〉 테이블에서 16학번인 학생의 학번과 이름, 학년을 가져오는 SQL문을 작성하시오. */
select 학번, 이름, 학년
from 학생
where 학번 like '16%'; /*where left(학번,2)='16'*/

/*=============38 <수강신청> 테이블에서 컴퓨터정보학과가 아닌 학생의 '수강신청번호', '학번','날짜'를 가져오는
SQL문을 작성하시오. =============*/
select 수강신청번호, 학생.학번, 날짜 -- 유일하면 테이블 표시 안해도 괜찮음.
from 수강신청, 학생, 학과
where 수강신청.학번=학생.학번
and 학과.학과번호=학생.학과 and 학과.학과명 <> '컴퓨터정보학과';

/*39 <수강신청> 테이블에서 2018년 3월에 수강 신청한 레코드의 '수강신청번호', 학번', '날짜'를 
가져오는 SQL문을 작성하시오. */
select 수강신청번호, 학번, 날짜
from 수강신청
where 날짜 like '2018-03%'; /*where year(날짜)=2018 and month(날짜)=03; ''가 없는것 날짜형*/

/*=============40 <수강신청내역> 테이블에서 평점이 입력되어 있는(평점'이 -1이 아닌) 레코드중 '수강신청번호',
 '과목번호', '평점'과 평점이 0이면 '미취득' 아니면 '취득'으로 출력하는 SQL문을 작성하시오. >제어문 사용 가능한지 묻는 문제 
 =============*/
 select 수강신청번호, 과목번호, 평점, if(평점=0, '미취득','취득') '취득여부' /*if(평점=0>조건,'미취득'>참,'취득'>거짓)*/
 from 수강신청내역
 where 평점 <> -1;
...
SQL문을 작성하시오*/
select 학번, substr(이름, 2,2) '이름' /*select 학번, right(이름,2) '이름'*/
from 학생;

/*45 <수강신청> 테이블에서 3월 1일 수강 신청한 데이터의 '학번'과 '날짜'를 출력하는 SQL문을 작성하시오*/
select 학번, 날짜
from 수강신청
where 날짜 like '%03-01%'; /*where month(날짜)=3 and day(날짜)=1;*/

/*=============46 <학생> 테이블에서 아래 그림과 같이 '학번', '이름', 학과'와 컴퓨터정보학과인지 
다른 학과인지를 출력하는 SQL문을 작성하시오.=============*/
select 학번, 이름, 학생.학과, if(학과.학과명='컴퓨터정보학과','컴퓨터정보학과','타과') '비고'
from 학생, 학과
where 학생.학과=학과.학과번호;
...
select 수강신청번호, count(*)'수강과목 수', avg(평점)'평균평점'
from 수강신청내역
where 평점 <>-1
group by 수강신청번호
having count(*)
...
SQL 문을 작성하시오.=============*/
select 수강신청.수강신청번호, 수강신청.과목번호, 과목명
from 수강신청, 학생
where 수강신청.학번=학생.학번 and 학번=1801001; 
...
select 수강신청.학번, 이름,count(*)'수강인원' avg(수강신청내역.평점)'평점평균'
from 학생, 수강신청, 수강신청내역, 과목
where 학생.학번=수강신청.학번 
...
select 과목.과번번호, 과목명, count(*) '수강인원',avg(평점)'평균평점'
from 과목, 수강신청내역
